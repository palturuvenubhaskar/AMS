  ...
  return (
    <div className="bg-white dark:bg-[#12141d] rounded-[32px] border border-slate-200 dark:border-[#222430] p-6 shadow-glass relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setDate(new Date(year, month + 1, 1))} className="p-1.5 hover:bg-slate-100 dark:bg-[#12141d] rounded-xl transition-colors"><ChevronRight size={16} className="text-gray-500 dark:text-gray-400" /></button>
      </div>
...
export default function FacultyDashboard() {
  ...
  const statCards = summary ? [
    { label: 'Total Students', value: summary.totalStudents, icon: Users, bgClass: 'bg-pastel-purple dark:bg-purple-500/10 dark:border-purple-500/20', iconColor: 'text-[#6941C6] dark:text-purple-400', text: 'text-slate-900 dark:text-slate-100', trend: '+2 this week' },
    { label: 'Present Today', value: summary.todayPresent, icon: CheckCircle, bgClass: 'bg-pastel-mint dark:bg-emerald-500/10 dark:border-emerald-500/20', iconColor: 'text-[#10b981] dark:text-emerald-400', text: 'text-slate-900 dark:text-slate-100', trend: 'Optimal' },
    { label: 'Absent Today', value: summary.todayAbsent, icon: XCircle, bgClass: 'bg-pastel-pink dark:bg-rose-500/10 dark:border-rose-500/20', iconColor: 'text-[#ef4444] dark:text-rose-400', text: 'text-slate-900 dark:text-slate-100', trend: '-1 from yesterday' },
    { label: 'Avg. Attendance', value: `${summary.avgAttendancePct}%`, icon: TrendingUp, bgClass: 'bg-pastel-peach dark:bg-amber-500/10 dark:border-amber-500/20', iconColor: 'text-[#f59e0b] dark:text-amber-400', isPercent: true, trend: 'Top 10% in dept' },
  ] : [];
    ...
    <div className="space-y-8 animate-[fadeIn_0.5s_ease-out]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-pastel-purple/30 dark:bg-purple-500/10 rounded-[32px] border border-pastel-purple dark:border-purple-500/20 p-8 shadow-glass">
          ...
          <button className="w-full mt-6 py-3.5 rounded-2xl bg-white dark:bg-[#12141d] border border-slate-200 dark:border-[#222430] text-gray-700 dark:text-gray-300 font-bold text-sm hover:border-brand-purple/30 hover:text-brand-purple hover:shadow-glass transition-all">
            View Full Timetable
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-6 lg:px-8 border-b border-gray-50">
          ...
          <div className="mt-4 sm:mt-0 relative w-full sm:w-72">
            <input type="text" placeholder="Search by name or roll no..."
              className="w-full h-11 pl-4 pr-10 rounded-2xl bg-slate-50 dark:bg-[#050505] border border-transparent text-sm text-slate-900 dark:text-slate-100 placeholder-gray-400 focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple/20 transition-all focus:outline-none"
              value={search} onChange={e => setSearch(e.target.value)} />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-white dark:bg-[#12141d] rounded-lg border border-slate-200 dark:border-[#222430] flex items-center justify-center text-[10px] font-bold text-gray-400 shadow-sm">/</div>
          </div>
        </div>
          ...
          <div className="relative bg-white dark:bg-[#12141d] rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col animate-[fadeIn_0.3s_ease-out]">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              ...
              <button 
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 dark:bg-[#12141d] hover:bg-slate-200 dark:bg-[#1a1c26] text-gray-500 dark:text-gray-400 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
