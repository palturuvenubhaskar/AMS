  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-gray-900 text-2xl font-black tracking-tight">Attendance History</h1>
          <p className="text-gray-500 text-sm font-medium mt-1">View all past attendance sessions</p>
        </div>
        <select 
          className="w-[240px] px-4 py-2.5 rounded-xl bg-white/60 backdrop-blur-md border border-white/50 text-gray-900 text-sm font-semibold shadow-glass focus:outline-none focus:ring-2 focus:ring-brand-purple/20 transition-all cursor-pointer appearance-none"
          value={classFilter} 
            ...
            <tbody className="divide-y divide-gray-100/50">
              {sessions.map(s => (
                <tr key={s.id} className="hover:bg-white/40 transition-colors">
                  <td className="px-6 py-4 text-sm font-semibold text-gray-900">{new Date(s.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-gray-900">{s.class_name}-{s.section}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-gray-600">{s.subject_name}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-gray-600">{s.time_slot}</td>
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-gray-600">{s.faculty_name}</td>
