    ...
    <div className="space-y-6 animate-[fadeIn_0.5s_ease-out] font-sans pb-12 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/60 backdrop-blur-2xl p-8 rounded-[32px] border border-white/60 shadow-glass">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-gray-900 text-3xl font-black tracking-tight mb-1">Users Directory</h1>
            <p className="text-gray-500 text-sm font-bold">Manage {users.length} registered accounts</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-48">
            <select 
              className="w-full appearance-none bg-white border border-transparent shadow-sm text-gray-900 text-sm font-bold rounded-2xl pl-4 pr-10 py-3 focus:outline-none focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple/20 transition-all cursor-pointer"
              value={roleFilter} 
          ...
          <button 
            className="bg-white hover:bg-gray-50 text-gray-900 border border-gray-200 px-6 py-3 rounded-2xl text-sm font-bold shadow-sm transition-all flex items-center gap-2 flex-shrink-0"
      ...
      {/* Data Table */}
      <div className="bg-white/60 backdrop-blur-2xl rounded-[32px] border border-white/60 shadow-glass overflow-hidden">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-white/40">
                {users.map(u => (
                  <tr key={u.id} className="group hover:bg-white/50 transition-colors">
                    <td className="px-6 py-5 font-black text-sm text-gray-400">#{u.id}</td>
                    <td className="px-6 py-5">
                      <div className="font-bold text-sm text-gray-900 group-hover:text-brand-purple transition-colors">{u.full_name}</div>
                    </td>
                    <td className="px-6 py-5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white/90 backdrop-blur-2xl rounded-[32px] shadow-glass border border-white w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-8 py-6 border-b border-white/50 flex justify-between items-center bg-white/40">
              <h3 className="font-black text-gray-900 text-xl">Add New User</h3>
              <button onClick={() => setModal(false)} className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-gray-400 hover:text-gray-900 transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={saveUser} className="p-8 space-y-5">
              <div>
                ...
                <input 
                  className="w-full bg-white border border-transparent shadow-sm text-gray-900 text-sm font-bold rounded-2xl px-4 py-3.5 focus:outline-none focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple/20 transition-all placeholder-gray-400"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <select 
                      className="w-full appearance-none bg-white border border-transparent shadow-sm text-gray-900 text-sm font-bold rounded-2xl pl-4 pr-10 py-3.5 focus:outline-none focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple/20 transition-all cursor-pointer"
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4">
                <button type="button" className="px-6 py-3.5 rounded-2xl text-sm font-bold bg-white border border-white text-gray-500 hover:bg-gray-50 shadow-sm transition-colors" onClick={() => setModal(false)}>
                  Cancel
                </button>
      ...
      {importModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white/90 backdrop-blur-2xl rounded-[32px] shadow-glass border border-white w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-8 py-6 border-b border-white/50 flex justify-between items-center bg-white/40">
              <h3 className="font-black text-gray-900 text-xl">Bulk Import Students</h3>
              <button onClick={() => setImportModal(false)} className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-gray-400 hover:text-gray-900 transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleBulkImport} className="p-8 space-y-5">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-2 font-medium">CSV must have headers: name, roll_no, email(optional), phone(optional)</p>
              </div>
              
              <div className="flex justify-end gap-3 mt-8 pt-4">
                <button type="button" className="px-6 py-3.5 rounded-2xl text-sm font-bold bg-white border border-white text-gray-500 hover:bg-gray-50 shadow-sm transition-colors" onClick={() => setImportModal(false)}>
                  Cancel
                </button>
                <button type="submit" className="px-6 py-3.5 rounded-2xl text-sm font-bold bg-gray-900 hover:bg-gray-800 text-white shadow-glass transition-colors">
                  Import CSV
