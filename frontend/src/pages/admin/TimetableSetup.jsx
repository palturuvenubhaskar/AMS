...
export default function TimetableSetup() {
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.5s_ease-out] font-sans pb-12">
        ...
        <div className="flex items-center gap-4">
          {view === 'timetable' && (
            <button 
              ...
              className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-[#111111] hover:bg-slate-50 dark:bg-black text-gray-500 dark:text-gray-400 hover:text-brand-purple shadow-sm transition-all"
            >
              <ArrowLeft size={24} />
            </button>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {classes.map(cls => (
            <div 
              ...
              className="bg-white/60 dark:bg-[#111111]/60 backdrop-blur-md p-8 rounded-[32px] border border-white/60 dark:border-gray-700/60 shadow-glass hover:shadow-glass-hover hover:-translate-y-1 hover:border-white transition-all cursor-pointer group"
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="bg-white dark:bg-[#111111] text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-sm">
                  Sem {cls.semester}
                </div>
              </div>
      ...
      {view === 'timetable' && (
        <div className="bg-white/60 dark:bg-[#111111]/60 backdrop-blur-2xl rounded-[32px] border border-white/60 dark:border-gray-700/60 shadow-glass overflow-hidden animate-[fadeIn_0.3s]">
          {/* Toolbar */}
          <div className="p-8 border-b border-white/40 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white/40 dark:bg-[#111111]/40">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="px-6 py-3 rounded-2xl text-sm font-bold bg-white dark:bg-[#111111] text-gray-700 dark:text-gray-300 hover:bg-slate-50 dark:bg-black shadow-sm border border-white transition-all flex items-center gap-2 hover:shadow-md"
              >
            ...
            {periods.length === 0 ? (
              ...
              <table className="w-full border-separate border-spacing-2 min-w-[800px]">
                ...
                <tbody>
                  {DAYS.map(day => (
                    <tr key={day}>
                      <td className="p-4 bg-white dark:bg-[#111111] rounded-2xl shadow-sm text-center border border-white/60 dark:border-gray-700/60 font-black text-gray-500 dark:text-gray-400 text-sm">{DAY_NAMES[day].slice(0, 3)}</td>
                      {periods.map((_, idx) => {
      ...
      {setupModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white/80 dark:bg-[#111111]/80 backdrop-blur-2xl rounded-[32px] shadow-glass border border-white w-full max-w-3xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {setupPeriods.map((p, idx) => (
                  <div key={idx} className="bg-white dark:bg-[#111111] p-5 rounded-3xl shadow-sm border border-white/50 hover:shadow-glass-hover transition-all">
            ...
            <div className="p-6 border-t border-white/50 bg-white/40 dark:bg-[#111111]/40 flex justify-end gap-3">
              <button onClick={() => setSetupModal(false)} className="px-6 py-3 rounded-2xl text-sm font-bold text-gray-600 dark:text-gray-400 bg-white dark:bg-[#111111] hover:bg-slate-50 dark:bg-black shadow-sm border border-white transition-all">Cancel</button>
