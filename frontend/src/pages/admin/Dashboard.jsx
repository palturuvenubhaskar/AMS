  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-10 md:w-12 h-10 md:h-12 border-4 border-slate-300 dark:border-slate-600 border-t-purple-500 rounded-full animate-spin drop-shadow-md" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-[fadeIn_0.5s_ease-out]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-slate-900 dark:text-slate-100 text-xl md:text-3xl font-black tracking-tight mb-1">System Overview</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 md:gap-6">
        {statCards.map((card, i) => {
          ...
          return (
            <div key={i} className="bg-white dark:bg-[#12141d] rounded-[24px] border border-slate-200 dark:border-[#222430] p-4 md:p-6 flex flex-col shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
              ...
              <div className="flex items-start justify-between mb-6">
                <div className={`w-10 md:w-14 h-10 md:h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br ${card.gradient} shadow-sm transform group-hover:scale-105 transition-transform duration-300`}>
                  <Icon size={26} className={card.iconColor} strokeWidth={2} />
                </div>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-slate-50 dark:bg-[#050505] px-2 py-1 rounded-md">{card.trend}</span>
              </div>
