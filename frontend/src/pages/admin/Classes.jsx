    ...
    <div className="space-y-6 animate-[fadeIn_0.5s_ease-out] font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-[24px] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-gray-900 text-2xl font-black tracking-tight mb-1">Classes</h1>
            <p className="text-gray-500 text-sm font-medium">Manage {classes.length} class sections</p>
          </div>
        </div>
              ...
              <tbody>
                {classes.map(c => (
                  <tr key={c.id} className="group hover:bg-indigo-50/30 transition-colors border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-6 py-4 font-bold text-sm text-gray-900">{c.name}</td>
                    <td className="px-6 py-4 font-bold text-sm text-gray-500">{c.section}</td>
                    <td className="px-6 py-4 font-bold text-sm text-gray-500">Sem {c.semester}</td>
                    <td className="px-6 py-4 font-bold text-sm text-gray-500">{c.academic_year}</td>
                    <td className="px-6 py-4 text-center font-black text-sm text-indigo-600">{c.student_count}</td>
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        <button 
                          className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-50 hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 transition-colors"
                        ...
                        <button 
                          className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors"
                          onClick={() => del(c.id)}
                        >
                          <Trash2 size={16} />
                        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-palette-dark/60 backdrop-blur-sm p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="font-black text-gray-900">{editing ? 'Edit' : 'Add'} Class</h3>
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                  <select 
                    className="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-900 text-sm font-semibold rounded-xl pl-4 pr-10 py-3 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm font-semibold rounded-xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="number" className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm font-semibold rounded-xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
              ...
              <div className="flex justify-end gap-3 mt-8">
                <button type="button" className="px-5 py-2.5 rounded-xl text-sm font-bold bg-gray-50 text-gray-500 hover:bg-gray-100 transition-colors" onClick={() => setModal(false)}>
                  Cancel
                </button>
