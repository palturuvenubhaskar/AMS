...
export default function DisputesInbox() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Disputes Inbox</h1>
        <p className="text-gray-500 font-medium mt-1">Review student attendance appeals and justifications.</p>
      </div>

      <div className="bg-white rounded-3xl shadow-glass border border-gray-100 overflow-hidden">
              ...
              {disputes.map(d => (
                <tr key={d.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-gray-900">{new Date(d.date).toLocaleDateString()}</div>
                    <div className="text-xs text-gray-500">{d.time_slot}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-gray-900">{d.student_name}</div>
                    <div className="text-xs text-gray-500">{d.roll_no}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-gray-900">{d.subject_name}</div>
                    <div className="text-xs text-gray-500">{d.class_name}-{d.section}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-700 max-w-xs truncate" title={d.reason}>{d.reason}</div>
                    {d.proof_url && (
