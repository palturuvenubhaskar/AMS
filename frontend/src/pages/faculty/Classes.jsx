...
export default function FacultyClasses() {
  ...
  return (
    <div>
      <h1 className="text-gray-900 text-2xl font-black tracking-tight mb-8">Classes & Students</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {classes.map(c => (
          <div 
            key={c.id} 
            className={`cursor-pointer bg-white/60 backdrop-blur-md rounded-3xl p-6 shadow-glass hover:bg-white hover:scale-[1.02] transition-all border-2 ${selectedClass===c.id ? 'border-brand-purple' : 'border-white/50'}`} 
            onClick={() => viewStudents(c.id)}
          >
            <h3 className="text-gray-900 font-bold text-lg">{c.department_code} {c.name}-{c.section}</h3>
            <p className="text-gray-500 text-sm font-medium mt-1">Semester {c.semester} — {c.academic_year}</p>
      ...
      {selectedClass && (
        <div className="bg-white/60 backdrop-blur-xl border border-white/50 rounded-3xl shadow-glass p-6">
          <h3 className="text-gray-900 font-bold text-lg mb-4">Enrolled Students</h3>
              ...
              <tbody className="divide-y divide-gray-100/50 bg-white/20">
                {students.map((s, i) => (
                  <tr key={s.id} className="hover:bg-white/40 transition-colors">
                    <td className="px-6 py-4 text-sm font-semibold text-gray-500">{i+1}</td>
                    <td className="px-6 py-4 text-sm font-mono font-medium text-gray-700">{s.roll_no}</td>
                    ...
                    <td className="px-6 py-4 text-sm font-black text-gray-900">{s.attendance_pct}%</td>
