    ...
    <div className="space-y-6 animate-[fadeIn_0.5s_ease-out] font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-[24px] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-gray-900 text-2xl font-black tracking-tight mb-1">Subjects</h1>
            <p className="text-gray-500 text-sm font-medium">Manage {subjects.length} course subjects</p>
          </div>
        </div>
              ...
              <tbody>
                {subjects.map(s => (
                  <tr key={s.id} className="group hover:bg-indigo-50/30 transition-colors border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-6 py-4 font-black font-mono text-xs text-gray-600 tracking-wider">{s.code}</td>
                    <td className="px-6 py-4 font-bold text-sm text-gray-900">{s.name}</td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-palette-dark/60 backdrop-blur-sm p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="font-black text-gray-900">{editing ? 'Edit' : 'Add'} Subject</h3>
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Subject Name</label>
                <input 
                  className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm font-semibold rounded-xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
